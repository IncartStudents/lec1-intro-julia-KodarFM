...
const PI = 3.14159
PI = 3.14
# Ответ: попытка изменения константы, запись нового значения не произойдёт и терминал выдаст ошибку

# Что происходит с типами глобальных переменных ниже, какого типа `c` и почему?
a = 1
b = 2.0
c = a + b
# Ответ: c - Float64, так как мы суммируем int и float происходит согласование типов данных, int переходит в float

# Что теперь произошло с переменной а? Как происходит биндинг имен в Julia?
a = "foo"
# Ответ: данные в переменной а перезаписались, данные типа int заменились на string, биндинг значения "foo" ПРИСВАИВАЕТСЯ переменной с именем "а" с изначальными данными "1"
...
greet()
# Ответ: в данном коде данные в переменной g ограниченны только типом int, что не позволяет заменить данные на string
...
v = "hello"
z
# Ответ: в 3 строке произошла замена значения первого элемента массива v - это присваивание, в то время как в 4 строчке данные переменной v - массив, заменились на данные типа string
